<template>
  <div class="virtual-pet-page">
    ...
    <div class="steps-container">
      ...
      <p class="carbon-info" v-if="carbonReduction && typeof carbonReduction === 'number'">减少碳排放: {{ carbonReduction.toFixed(2) }} kg CO2</p>
      <p class="coins-info" v-if="coinsEarned">获得碳币: {{ coinsEarned }}</p>
      
      <div v-if="!hasSubmittedStepsToday" class="steps-input">
      ...
      <div v-else class="steps-submitted">
        <div class="submitted-message">
          <i class="fas fa-check-circle"></i>
          <span>今日已提交 {{ todaySteps }} 步</span>
        </div>
        <div class="next-day-hint">请明天再来提交新的步数！</div>
      </div>
    </div>
  ...
  methods: {
    ...
    async submitSteps() {
      try {
        ...
        // 显示成功消息
        this.$store.dispatch('message/showMessage', {
          content: `步数提交成功！减少了${typeof response.data.carbonReduction === 'number' ? response.data.carbonReduction.toFixed(2) : '0.00'}kg碳排放，获得了${response.data.coinsEarned}碳币！`,
          type: 'success'
        });
